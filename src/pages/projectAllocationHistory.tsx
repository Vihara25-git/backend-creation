...
import { ChevronLeft, Users, User, ChevronDown, ChevronUp, Search, Filter } from 'lucide-react';
...
useEffect(() => {
  ...
  console.log("Allocation Historyqqq:", allocationHistory);
 ...
 const currentProject = useMemo(
  () => accessibleProjects.find(p => String(p.id) === String(selectedProjectId)),
...
const handleProjectSelect = (id: string) => {
  ...
  return allocationHistory.filter(user => {
    const records = [...(user.allocations || []), ...(user.deallocations || [])];
    ...
    const roleMatch =
      roleFilter === 'all' ||
      records.some(record => String(record.roleId) === String(roleFilter));

    return searchMatch && roleMatch;
  });
}, [allocationHistory, searchTerm, roleFilter]);
...
<Card className="mb-6">
  ...
  <Select.Item
    key={r.id}
    value={String(r.id)}
    className="pl-4 pr-8 py-2 flex items-center cursor-pointer data-[state=checked]:bg-blue-100"
  >
